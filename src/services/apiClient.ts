  ...
  async update(id: string, data: Partial<Artist>): Promise<Artist> {
    try {
      ...
      const mappedArtist: Artist = {
        ...
        verificationStatus: artist.verificationStatus || data.verificationStatus || "",
        verificationNote: artist.verificationNote || data.verificationNote || "",
  ...
  async getBookings(): Promise<Booking[]> {
    try {
      ...
      const mappedBookings = data.bookings.map((booking: any) => ({
        ...
        advanceAmount: booking.advanceAmount ?? (typeof booking.serviceId === 'object' ? booking.serviceId?.advance : 0) ?? 0,
        paidAmount: booking.paidAmount ?? (booking.status === 'confirmed' || booking.status === 'completed' ? (booking.advanceAmount || (typeof booking.serviceId === 'object' ? booking.serviceId?.advance : 0)) : 0) ?? 0,
  ...
  async create(booking: Omit<Booking, "id" | "createdAt" | "updatedAt">): Promise<Booking> {
    try {
      ...
      const mappedBooking: Booking = {
        ...
        advanceAmount: createdBooking.advanceAmount || booking.advanceAmount || 0,
        paidAmount: createdBooking.paidAmount || booking.paidAmount || 0,
